...
async function build() {
	...
	const sourceCode = (await bundle(entry)).content;
	await utils.wait(1000);
	commonjsCompiler(sourceCode, entry, [bannerText(licenseText)]);
	await utils.wait(2000);
	esmCompiler(sourceCode, entry, [bannerText(licenseText)]);
